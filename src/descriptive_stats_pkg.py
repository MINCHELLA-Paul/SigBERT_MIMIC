...
def print_dataset_statistics(
    ...
    """
    Compute and print descriptive statistics for a longitudinal
    survival dataset.

    Parameters
    ----------
    df : pd.DataFrame
        Input dataframe containing repeated observations for each patient.
    ...
    required_columns = {var_id, var_death}
    ...
    patient_event = df.groupby(var_id, observed=True)[var_death].max()

    num_unique_patients = df[var_id].nunique()
    num_total_reports = len(df)

    reports_per_patient = df.groupby(var_id,observed=True).size()

    mean_reports_per_patient = reports_per_patient.mean()

    num_deceased = int((patient_event == 1).sum())
    num_censored = int((patient_event == 0).sum())

    print(f"Total number of patients: {num_unique_patients}")
    print(f"Total number of medical reports: {num_total_reports}")
    print(
        "Average number of reports per patient: "
        f"{mean_reports_per_patient:.2f}"
    )
...
def make_df_info(
    ...
    """
    Build a patient-level dataframe from longitudinal observations.

    For each patient:
    - `date_start` is the date of the first available report;
    - `date_end` is the date of the last available report;
    - `duration` is the time between both dates, expressed in days;
    - the remaining patient-level variables are taken from the last
      chronological observation.

    Parameters
    ----------
    df : pd.DataFrame
        Longitudinal dataframe containing several reports per patient.

    columns_to_keep : list of str or None, default=None
        Columns retained in the final dataframe. The derived columns
        `date_start`, `date_end`, and `duration` can be included.
        If None, all columns from the last patient observation are retained.
    ...
    var_start : str, default="date_start"
        Name assigned to the first-report date column.

    var_end : str, default="date_end"
        Name assigned to the last-report date column.

    var_duration : str, default="duration"
        Name assigned to the report follow-up duration column.
    ...
    required_columns = {var_id, var_time}
    missing_columns = required_columns.difference(df.columns)
    ...
    # Ensure that report dates have a datetime format
    df_work[var_time] = pd.to_datetime(
        df_work[var_time],
        errors="coerce"
    )

    if df_work[var_time].isna().any():
        n_missing_dates = int(df_work[var_time].isna().sum())

        raise ValueError(
            f"{n_missing_dates} rows contain an invalid or missing "
            f"value in `{var_time}`."
        )

    # Sort reports chronologically within each patient
    df_work = df_work.sort_values(
        by=[var_id, var_time]
    )

    # First and last report dates for each patient
    df_dates = (
        df_work.groupby(var_id, as_index=False, observed=True)
        .agg(
            **{
                var_start: (var_time, "min"),
                var_end: (var_time, "max")
            }
        )
    )

    # Retain the last chronological observation for patient-level variables
    df_last = (
        df_work
        .drop_duplicates(subset=var_id, keep="last")
        .copy()
    )

    # Add first and last report dates
    df_info = df_last.merge(
        df_dates,
        on=var_id,
        how="left",
        validate="one_to_one"
    ...
    # Duration covered by the available reports
    df_info[var_duration] = (
        df_info[var_end] - df_info[var_start]
    ).dt.total_seconds() / (24 * 3600)

    # Select requested columns only after deriving patient-level information
    if columns_to_keep is not None:
        final_columns = list(dict.fromkeys(
            [var_id] + columns_to_keep
        ))

        missing_output_columns = set(final_columns).difference(
            df_info.columns
        )

        if missing_output_columns:
            raise ValueError(
                "Columns requested in `columns_to_keep` are missing: "
                f"{sorted(missing_output_columns)}"
            )

        df_info = df_info[final_columns].copy()

    df_info = df_info.reset_index(drop=True)

    if verbose:
        print(f"Number of patients: {df_info[var_id].nunique()}")
        print(
            "Mean observation period: "
            f"{df_info[var_duration].mean():.2f} days"
        )
        print(
            "Median observation period: "
            f"{df_info[var_duration].median():.2f} days"
        )
...
def plot_km_train_vs_validation_with_logrank(
    ...
    """
    Plot Kaplan-Meier curves for training and validation cohorts,
    perform a log-rank test, and report median duration among event cases.

    Parameters
    ----------
    df_info : pandas.DataFrame
        Patient-level dataframe containing survival information.
        One row per patient is recommended.

    id_column : str
        Patient identifier column in `df_info`.

    duration_column : str
        Observed survival duration column in `df_info`.

    event_column : str
        Event indicator column in `df_info`:
        - 1: event observed;
        - 0: censored.

    df_train : pandas.DataFrame
        Training dataframe containing patient identifiers.

    test_groups : list of pandas.DataFrame
        Validation/test dataframes containing patient identifiers.

    train_id_column : str or None, default=None
        Patient identifier column in `df_train`.
        If None, `id_column` is used.

    test_id_column : str or None, default=None
        Patient identifier column in each dataframe of `test_groups`.
        If None, `id_column` is used.

    train_label : str, default="Train"
        Label assigned to the training Kaplan-Meier curve.

    validation_label : str, default="Validation"
        Label assigned to the validation Kaplan-Meier curve.
    ...
    xlim : tuple or None, default=None
        Optional limits of the horizontal axis.
    ...
    verbose : bool, default=True
        Whether to print cohort and log-rank statistics.

    Returns
    -------
    df_info_train : pandas.DataFrame
        Patient-level survival information for the training cohort.

    df_info_val : pandas.DataFrame
        Patient-level survival information for the validation cohort.

    logrank_results : lifelines.statistics.StatisticalResult
        Log-rank test results.

    median_event_time_train : float
        Median observed duration among training patients with an event.

    median_event_time_val : float
        Median observed duration among validation patients with an event.
    """

    train_id_column = (
        id_column
        if train_id_column is None
        else train_id_column
    )

    test_id_column = (
        id_column
        if test_id_column is None
        else test_id_column
    )
    ...
    missing_info_columns = required_info_columns.difference(
        df_info.columns
    )

    if missing_info_columns:
        raise ValueError(
            "Missing columns in `df_info`: "
            f"{sorted(missing_info_columns)}"
        )

    if train_id_column not in df_train.columns:
        raise ValueError(
            f"`{train_id_column}` is missing from `df_train`."
        )
    ...
    if len(test_groups) == 0:
        raise ValueError(
            "`test_groups` must contain at least one dataframe."
        )

    # --------------------------------------------------------------
    # Recover training and validation patient identifiers
    # --------------------------------------------------------------
    train_ids = set(
        df_train[train_id_column]
        .dropna()
        .unique()
    )
    ...
    for group_index, df_test in enumerate(test_groups):
        if test_id_column not in df_test.columns:
            raise ValueError(
                f"`{test_id_column}` is missing from "
                f"`test_groups[{group_index}]`."
            )

        validation_ids.update(
            df_test[test_id_column]
            .dropna()
            .unique()
        )
    ...
    if overlapping_ids:
        raise ValueError(
            f"{len(overlapping_ids)} patient IDs occur in both "
            "training and validation cohorts."
        )

    # --------------------------------------------------------------
    # Extract patient-level survival data
    # --------------------------------------------------------------
    df_info_train = df_info.loc[
        df_info[id_column].isin(train_ids)
    ].copy()

    df_info_val = df_info.loc[
        df_info[id_column].isin(validation_ids)
    ].copy()

    # Keep one survival record per patient
    df_info_train = (
        df_info_train
        .drop_duplicates(subset=id_column, keep="last")
        .dropna(subset=[duration_column, event_column])
    ...
    df_info_train = df_info_train.loc[
        df_info_train[duration_column] >= 0
    ].copy()

    df_info_val = df_info_val.loc[
        df_info_val[duration_column] >= 0
    ].copy()

    if df_info_train.empty:
        raise ValueError(
            "No valid training patient was found in `df_info`."
        )

    if df_info_val.empty:
        raise ValueError(
            "No valid validation patient was found in `df_info`."
        )

    invalid_event_values = (
        set(df_info_train[event_column].unique())
        .union(set(df_info_val[event_column].unique()))
        .difference({0, 1, False, True})
    )

    if invalid_event_values:
        raise ValueError(
            f"`{event_column}` must contain only 0 and 1. "
            f"Invalid values: {sorted(invalid_event_values)}"
        )
    ...
    kmf_train.fit(
        durations=df_info_train[duration_column],
        event_observed=df_info_train[event_column],
        label=train_label
    )

    kmf_val.fit(
        durations=df_info_val[duration_column],
    ...
    # --------------------------------------------------------------
    # Median observed duration among patients with an event
    # --------------------------------------------------------------
    median_event_time_train = df_info_train.loc[
        df_info_train[event_column] == 1,
        duration_column
    ].median()

    median_event_time_val = df_info_val.loc[
        df_info_val[event_column] == 1,
        duration_column
    ].median()

    if verbose:
        print(f"Train cohort size: {len(df_info_train)} patients")
        print(
            f"Validation cohort size: "
            f"{len(df_info_val)} patients"
        )
        print(
            f"Train events: "
            f"{int(df_info_train[event_column].sum())}"
        )
        print(
            f"Validation events: "
            f"{int(df_info_val[event_column].sum())}"
        )
        ...
        print(
            f"Log-rank p-value: "
            f"{logrank_results.p_value:.4g}"
        )

        print(
            f"\nMedian event time ({train_label}): "
            f"{median_event_time_train:.2f} days"
        )
        print(
            f"Median event time ({validation_label}): "
